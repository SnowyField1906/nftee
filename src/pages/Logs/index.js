  ...
  return (
    <>
      ...
      <div className='page-bg h-screen  main-overflow select-none'>
        ...
        <div className='flex absolute bottom-5 z-20 '>
          <button className={`${page === 0 ? 'button-medium-disabled cursor-not-allowed' : 'button-medium cursor-pointer'} w-32 text-center text-medium text-xl h-fit py-3 px-5 rounded-md`} disabled={page <= 0}
            onClick={() => setPage(page - 1)}>Previous</button>
          <div className='text-medium text-xl h-fit py-3 px-5 rounded-md'>Page {page + 1} ({20 * page + 20 < notifications.length ? 20 * page + 20 : notifications.length}/{notifications.length})</div>
          <button className={`${20 * page + 20 > notifications.length ? 'button-medium-disabled cursor-not-allowed' : 'button-medium cursor-pointer'} w-32 text-center text-medium text-xl h-fit py-3 px-5 rounded-md`} disabled={20 * page + 20 > notifications.length}
